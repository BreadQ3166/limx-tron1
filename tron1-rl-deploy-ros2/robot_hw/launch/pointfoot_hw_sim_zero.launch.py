    ...
    return LaunchDescription([
        ...
        Node(
            ...
            output='screen',
               ...
               # 新增的激光雷达里程计节点
        Node(
            package='lidar_odometry',          # 功能包名
            executable='lidar_odometry_node',  # 可执行文件/节点名
            name='lidar_odometry_node',        # 节点实例名（可自定义）
            output='screen',                   # 输出到终端
            # 可选：添加参数（根据实际需求配置）
            parameters=[
                {
                    "use_gazebo": LaunchConfiguration("use_gazebo")},  # 共享gazebo标志
            ],
        ),
    ])